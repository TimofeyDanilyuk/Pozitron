...
const showContextMenu = (event: TouchEvent | MouseEvent, msg: any) => {
  ...
  contextMenu.value = { x: Math.min(clientX, window.innerWidth - 168), y: Math.min(clientY, window.innerHeight - 120), msg };
  reactionPicker.value = null;
};
const closeContextMenu = () => { contextMenu.value = null; reactionPicker.value = null; };
...
const onStickerClick = async (packId: string) => {
  ...
  try { stickerPackModal.value = await chat.getPack(packId); }
  finally { stickerPackModalLoading.value = false; }
};
...
const handleToggleStickerPicker = async () => {
  isStickerPickerOpen.value = !isStickerPickerOpen.value;
  isChatEmojiPickerOpen.value = false;
  if (isStickerPickerOpen.value && !chat.stickerPacksLoaded) await chat.loadMyPacks();
};
...
const filteredAllUsers = computed(() => {
  ...
  return chat.allUsers.filter(u => u.username.toLowerCase().includes(contactSearch.value.toLowerCase()));
});
...
const confirmAddContacts = async () => {
  ...
  try {
    for (const userId of selectedUserIds.value) await chat.addContact(userId);
    isAddContactsOpen.value = false;
  } finally { addingContacts.value = false; }
};
...
const emojiCategories = [
  ...
  { label: '❤️ Сердца', emojis: ['❤️','🧡','💛','💚','💙','💜','🖤','🤍','💔','💕','💞','💓','💗','💖','💘','💝'] },
  { label: '🌟 Символы', emojis: ['✨','⭐','🌟','💫','⚡','🔥','🌈','☄️','🌊','💥','🎉','🎊','🎁','🏆','🎯','🎲','🎨'] },
];
const filteredProfileEmojis = computed(() => {
...
const scrollToBottomSmooth = () => messagesEnd.value?.scrollIntoView({ behavior: 'smooth' });
...
const handleLogout = () => { chat.disconnect(); auth.logout(); router.push('/'); };
const onSearch = async () => {
  if (searchQuery.value.trim()) { await chat.searchUsers(searchQuery.value); isSearchOpen.value = true; }
  else isSearchOpen.value = false;
};
const startDm = async (userId: string) => {
  searchQuery.value = ''; isSearchOpen.value = false;
  await chat.openDm(userId); mobileView.value = 'chat';
};
const openProfile = () => {
...
const saveProfile = async () => {
  usernameError.value = '';
  if (newUsername.value.trim().length < 3) { usernameError.value = 'Ник должен быть не короче 3 символов'; return; }
  isSaving.value = true;
  try {
    if (newUsername.value.trim() !== auth.user?.username) await auth.changeUsername(newUsername.value.trim());
    await auth.updateProfile({ emojiPrefix: userPrefix.value, displayName: newUsername.value.trim() });
...
const onFileSelected = async (event: Event) => {
  ...
  if (target.files && target.files[0]) {
    try { await auth.uploadAvatar(target.files[0]); }
  ...
  <div class="flex h-screen bg-white dark:bg-slate-900 text-slate-900 dark:text-slate-100 overflow-hidden font-sans select-none transition-colors duration-300">
    ...
    <aside :class="[
      ...
      <div class="p-4 border-b border-slate-200 dark:border-slate-800 flex justify-between items-center bg-white/80 dark:bg-slate-950/30 shrink-0">
        <div class="flex items-center gap-3 overflow-hidden">
          ...
          <div class="flex items-center gap-1.5 min-w-0">
            ...
            <h2 class="text-base font-black tracking-tight truncate uppercase italic text-slate-900 dark:text-white">{{ auth.user?.username || 'User' }}</h2>
          </div>
        </div>
        <div class="flex items-center gap-2 shrink-0">
          <button @click="toggleTheme" class="w-10 h-10 rounded-xl bg-slate-100 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700 flex items-center justify-center hover:bg-slate-200 dark:hover:bg-slate-700 transition-all active:scale-90">
          ...
          <div @click="openProfile" class="relative group cursor-pointer transition-transform active:scale-90">
            <img v-if="auth.user?.avatarUrl" :src="auth.user.avatarUrl" class="w-10 h-10 rounded-xl border-2 border-purple-500 object-cover">
            <div v-else class="w-10 h-10 rounded-xl bg-gradient-to-tr from-purple-600 to-indigo-600 flex items-center justify-center font-bold border-2 border-white/10 text-white">{{ auth.user?.username?.[0]?.toUpperCase() }}</div>
          </div>
          <button v-if="auth.user?.role === 1" @click="router.push('/admin')" class="w-10 h-10 rounded-xl bg-purple-600/20 border border-purple-500/30 flex items-center justify-center hover:bg-purple-600/40 transition-all active:scale-90">
        ...
        <template v-if="chat.contactChats.length > 0">
          ...
          <div v-for="c in chat.contactChats" :key="c.id" @click="openChat(c)"
            ...
            <div class="relative shrink-0">
              ...
              <div v-else class="w-12 h-12 rounded-xl bg-slate-200 dark:bg-slate-700 flex items-center justify-center text-xl">{{ c.name?.[0]?.toUpperCase() }}</div>
            </div>
            <div class="min-w-0 flex-1">
              <div class="flex items-center gap-1.5">
                <svg viewBox="0 0 16 16" class="w-3 h-3 text-purple-400 shrink-0 fill-current"><path d="M8 8a3 3 0 1 0 0-6 3 3 0 0 0 0 6zm0 1a5 5 0 0 0-5 5h10a5 5 0 0 0-5-5z"/></svg>
            ...
            <span v-if="c.unreadCount > 0" class="shrink-0 min-w-5 h-5 px-1 bg-purple-500 text-white text-xs font-bold rounded-full flex items-center justify-center">{{ c.unreadCount > 99 ? '99+' : c.unreadCount }}</span>
            <span class="text-slate-300 dark:text-slate-600 md:hidden flex items-center">
              <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" class="w-5 h-5" fill="none"><path d="M9 6L15 12L9 18" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"/></svg>
            </span>
          </div>
        </template>
        ...
        <template v-if="chat.otherChats.length > 0">
          ...
          <div v-for="c in chat.otherChats" :key="c.id" @click="openChat(c)"
            ...
            <div class="relative shrink-0">
              ...
              <div v-else class="w-12 h-12 rounded-xl bg-slate-200 dark:bg-slate-700 flex items-center justify-center text-xl">{{ c.type === 0 ? '🌐' : c.name?.[0]?.toUpperCase() }}</div>
            </div>
            <div class="min-w-0 flex-1">
      ...
      <div class="p-4 border-t border-slate-200 dark:border-slate-800 shrink-0 space-y-2">
        <button @click="openAddContacts" class="w-full flex items-center gap-2 px-4 py-2.5 rounded-xl bg-purple-600/10 border border-purple-500/20 hover:bg-purple-600/20 transition-all active:scale-95 text-purple-500 dark:text-purple-400 text-sm font-bold">
          <svg viewBox="0 0 24 24" class="w-4 h-4 fill-current shrink-0"><path d="M19 11h-6V5a1 1 0 0 0-2 0v6H5a1 1 0 0 0 0 2h6v6a1 1 0 0 0 2 0v-6h6a1 1 0 0 0 0-2z"/></svg>
          Добавить контакты...
        </button>
        <div class="relative">
          ...
          <div v-if="isSearchOpen" class="absolute bottom-full mb-2 left-0 right-0 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl overflow-hidden shadow-2xl max-h-48 overflow-y-auto">
            <div v-for="user in chat.users" :key="user.id" @click="startDm(user.id)" class="flex items-center gap-3 px-4 py-3 hover:bg-slate-100 dark:hover:bg-slate-800 active:bg-slate-200 dark:active:bg-slate-700 cursor-pointer transition-all">
    ...
    <main :class="['flex-1 flex flex-col bg-slate-50 dark:bg-slate-900 relative', mobileView === 'list' ? 'hidden md:flex' : 'flex']">
      ...
      <header class="px-4 py-3 border-b border-slate-200 dark:border-slate-800 flex items-center justify-between bg-white/80 dark:bg-slate-900/80 backdrop-blur-md z-10 shrink-0">
        <div class="flex items-center gap-2">
          <button @click="goBackToList" class="md:hidden w-9 h-9 flex items-center justify-center rounded-xl hover:bg-slate-100 dark:hover:bg-slate-800 active:bg-slate-200 dark:active:bg-slate-700 transition-all">
            <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none">
              <defs><linearGradient id="g" x1="0" y1="0" x2="24" y2="24"><stop offset="0%" stop-color="#7C5CFF"/><stop offset="100%" stop-color="#37C8FF"/></linearGradient></defs>
          ...
          <div v-if="chat.activeChat" class="flex items-center gap-2">
            <div class="w-8 h-8 rounded-lg bg-slate-200 dark:bg-slate-700 flex items-center justify-center text-base shrink-0">{{ chat.activeChat.type === 0 ? '🌐' : chat.activeChat.name?.[0]?.toUpperCase() }}</div>
            <div class="flex items-center gap-1.5">
              <p class="font-bold text-sm" :class="chat.activeChat.isContact ? 'text-purple-600 dark:text-purple-300' : ''">{{ chat.activeChat.name || 'Чат' }}</p>
              <svg v-if="chat.activeChat.isContact" viewBox="0 0 16 16" class="w-3.5 h-3.5 text-purple-400 fill-current"><path d="M8 8a3 3 0 1 0 0-6 3 3 0 0 0 0 6zm0 1a5 5 0 0 0-5 5h10a5 5 0 0 0-5-5z"/></svg>
            </div>
          </div>
        ...
        <button @click="handleLogout" class="px-3 py-2 rounded-xl bg-red-500/10 hover:bg-red-500/20 text-red-500 dark:text-red-400 text-xs font-bold transition-all border border-red-500/20 active:scale-95">Выйти</button>
      </header>
      ...
      <div ref="messagesContainer" @scroll="onMessagesScroll" class="flex-1 px-3 py-4 overflow-y-auto z-10 space-y-3 select-text relative">
        ...
        <div v-else-if="!chat.activeChat" class="flex h-full items-center justify-center opacity-20">
          <div class="text-center"><p class="text-4xl mb-2">💬</p><p class="text-sm italic font-medium">Выберите чат</p></div>
        </div>
        ...
        <template v-else>
          ...
          <div v-for="msg in currentMessages" :key="msg.id" :id="`msg-${msg.id}`"
            ...
            <div :class="['flex flex-col gap-0.5 max-w-xs sm:max-w-sm', msg.userId === auth.user?.id ? 'items-end' : 'items-start']">
              ...
              <div @contextmenu.prevent="showContextMenu($event, msg)"
                ...
                <MessageBubble
                  ...
                  :on-sticker-click="onStickerClick"
                  :on-reply="chat.setReply"
                  :on-react="(id) => { reactionPicker = { x: 0, y: 0, msgId: id }; }"
                  :scroll-to-message="scrollToMessage"
                  :add-reaction="chat.addReaction"
      ...
      <footer class="p-3 bg-white/80 dark:bg-slate-950/80 backdrop-blur-md border-t border-slate-200 dark:border-slate-800 z-10 shrink-0 relative">
        <Transition name="picker">
          <StickerPicker v-if="isStickerPickerOpen" @send-sticker="handleSendSticker" @close="isStickerPickerOpen = false" />
        </Transition>
        <Transition name="picker">
          <EmojiPicker v-if="isChatEmojiPickerOpen" class="mb-2" @select="(e) => { messageInput += e; isChatEmojiPickerOpen = false; }" />
        </Transition>
        ...
        <div class="flex items-end gap-2 bg-slate-100 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700/50 p-1.5 rounded-2xl focus-within:border-purple-500/50 transition-all">
              ...
              <div class="flex items-center gap-2 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl px-3 py-2 shadow-lg">
                ...
                <div class="flex-1 min-w-0">
                  ...
                  <p class="text-xs text-slate-500 dark:text-slate-400 truncate">
                    {{ chat.replyingTo.type === 'Voice' ? '🎤 Голосовое'
                       : chat.replyingTo.type === 'Image' ? '🖼️ Изображение'
                       : chat.replyingTo.type === 'Video' ? '📹 Видео'
                       : chat.replyingTo.type === 'Sticker' ? '🎭 Стикер'
                       : chat.replyingTo.content }}
                  </p>
                </div>
                <button @click="chat.setReply(null)" class="shrink-0 w-6 h-6 flex items-center justify-center rounded-lg hover:bg-slate-100 dark:hover:bg-slate-800 text-slate-400 transition-all">
                  <svg viewBox="0 0 24 24" class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="2.5"><path d="M18 6L6 18M6 6l12 12" stroke-linecap="round"/></svg>
                </button>
              </div>
            </div>
          </Transition>
          ...
          <button @click="triggerAttachmentInput" :disabled="!chat.activeChat || uploadingAttachment"
                  :class="['p-2 rounded-xl transition-colors shrink-0 self-end mb-0.5', !chat.activeChat ? 'opacity-40 cursor-not-allowed' : 'hover:bg-slate-200 dark:hover:bg-slate-700 text-slate-500 dark:text-slate-400']">
            ...
            <svg v-else xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none">
              <defs><linearGradient id="ga" x1="0" y1="0" x2="24" y2="24"><stop offset="0%" stop-color="#7C5CFF"/><stop offset="100%" stop-color="#37C8FF"/></linearGradient></defs>
              <path d="M21 11.5L12.5 20C10.3 22.2 6.7 22.2 4.5 20C2.3 17.8 2.3 14.2 4.5 12L13 3.5C14.4 2.1 16.6 2.1 18 3.5C19.4 4.9 19.4 7.1 18 8.5L9.5 17C8.8 17.7 7.7 17.7 7 17C6.3 16.3 6.3 15.2 7 14.5L14.5 7" stroke="url(#ga)" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round"/>
            </svg>
          </button>
          <input id="attachmentInput" type="file" class="hidden" accept="image/png,image/jpeg,image/gif,image/webp,video/mp4,video/webm" @change="onAttachmentSelected">

          <button @click.stop="handleToggleStickerPicker" :disabled="!chat.activeChat"
          ...
          <button v-if="!messageInput.trim() && chat.activeChat" @click="toggleRecording"
                  :class="['px-3 py-2.5 rounded-xl shadow-lg transition-all shrink-0 self-end active:scale-95', isRecording ? 'bg-red-500 animate-pulse' : 'bg-gradient-to-r from-purple-600 to-indigo-600']">
            ...
            <svg v-else viewBox="0 0 24 24" class="w-6 h-6 fill-white"><rect x="6" y="6" width="12" height="12" rx="2"/></svg>
          </button>
        </div>
      </footer>
    </main>
    ...
    <Transition name="fade">
      <div v-if="isAddContactsOpen" class="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-4">
        ...
        <div class="relative bg-white dark:bg-slate-900 border border-slate-200 dark:border-white/10 w-full sm:max-w-sm rounded-t-3xl sm:rounded-3xl overflow-hidden shadow-2xl">
          ...
          <div class="px-4 pt-3">
            <input v-model="contactSearch" type="text" placeholder="Поиск..." class="w-full bg-slate-100 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl px-4 py-2.5 text-sm outline-none focus:ring-1 focus:ring-purple-500 select-text text-slate-900 dark:text-slate-100">
          </div>
          <div class="max-h-64 overflow-y-auto p-2 mt-2">
            <div v-if="chat.allUsers.length === 0" class="flex items-center justify-center py-8 opacity-30"><p class="text-sm">Загрузка...</p></div>
            <div v-for="user in filteredAllUsers" :key="user.id" @click="toggleSelectUser(user.id)"
              ...
              <div v-else class="w-10 h-10 rounded-xl bg-gradient-to-tr from-purple-600 to-indigo-600 flex items-center justify-center text-sm font-bold shrink-0 text-white">{{ user.username?.[0]?.toUpperCase() }}</div>
              <div class="flex-1 min-w-0">
              ...
              <div :class="['w-6 h-6 rounded-lg border-2 flex items-center justify-center transition-all shrink-0', selectedUserIds.has(user.id) ? 'bg-purple-600 border-purple-600' : 'border-slate-300 dark:border-slate-600']">
                <svg v-if="selectedUserIds.has(user.id)" viewBox="0 0 16 16" class="w-3.5 h-3.5 fill-white"><path d="M13.5 3.5L6 11L2.5 7.5L1.5 8.5L6 13L14.5 4.5L13.5 3.5Z"/></svg>
              </div>
            </div>
          </div>
          <div class="p-4 border-t border-slate-200 dark:border-slate-800 flex gap-3">
            <button @click="isAddContactsOpen = false" class="flex-1 bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 text-slate-900 dark:text-white font-bold py-3 rounded-xl text-sm transition-all active:scale-95">Отмена</button>
            <button @click="confirmAddContacts" :disabled="selectedUserIds.size === 0 || addingContacts"
          ...
          <div v-else-if="stickerPackModal">
            ...
            <div class="p-4 border-t border-slate-200 dark:border-slate-800">
              <button @click="togglePackFromModal"
                      :class="['w-full font-bold py-3 rounded-xl text-sm active:scale-95 transition-all', stickerPackModal.isAdded ? 'bg-red-500/10 border border-red-500/20 text-red-500 dark:text-red-400 hover:bg-red-500/20' : 'bg-gradient-to-r from-purple-600 to-indigo-600 text-white shadow-lg']">
          ...
          <div class="px-6 pb-8">
            <div class="relative -mt-10 mb-4 flex justify-center">
              <div @click="triggerFileInput" class="relative group cursor-pointer">
                ...
                <div v-else class="w-20 h-20 rounded-2xl border-4 border-white dark:border-slate-900 bg-slate-200 dark:bg-slate-800 flex items-center justify-center text-2xl font-bold uppercase">{{ auth.user?.username?.[0] }}</div>
            ...
            <div class="text-center mb-5"><h3 class="text-xl font-black">{{ auth.user?.username }}</h3></div>
            <div class="space-y-4">
              <div>
                ...
                <input v-model="newUsername" type="text" placeholder="username" maxlength="32" @input="usernameError = ''"
                       :class="['w-full bg-slate-100 dark:bg-slate-800/50 border rounded-xl px-4 py-3 text-sm outline-none focus:ring-1 transition-all select-text text-slate-900 dark:text-slate-100', usernameError ? 'border-red-500/70 focus:ring-red-500' : 'border-slate-200 dark:border-slate-700 focus:ring-purple-500']">
              ...
              <div>
                ...
                <div class="flex gap-2">
                  <input v-model="userPrefix" type="text" placeholder="🚀" maxlength="2" class="flex-1 bg-slate-100 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700 rounded-xl px-4 py-3 text-center text-2xl outline-none focus:ring-1 focus:ring-purple-500 select-text">
                  <button @click.stop="isEmojiPickerOpen = !isEmojiPickerOpen"
                          :class="['px-4 rounded-xl border text-lg transition-all', isEmojiPickerOpen ? 'bg-purple-600/30 border-purple-500' : 'bg-slate-100 dark:bg-slate-800/50 border-slate-200 dark:border-slate-700']">🙂</button>
                </div>
                <Transition name="picker">
                  <div v-if="isEmojiPickerOpen" class="mt-2 bg-white dark:bg-slate-950 border border-slate-200 dark:border-slate-700 rounded-2xl overflow-hidden" @click.stop>
                    <div class="p-2 border-b border-slate-200 dark:border-slate-800">
                      <input v-model="emojiSearch" type="text" placeholder="Поиск..." class="w-full bg-slate-100 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg px-3 py-2 text-xs outline-none select-text text-slate-900 dark:text-slate-100">
                    </div>
                    <div class="max-h-44 overflow-y-auto p-2 space-y-3">
                      <div v-for="cat in filteredProfileEmojis" :key="cat.label">
                        <p class="text-[10px] font-bold text-slate-400 dark:text-slate-500 uppercase px-1 mb-1">{{ cat.label }}</p>
                        <div class="flex flex-wrap gap-0.5">
                          <button v-for="emoji in cat.emojis" :key="emoji" @click="userPrefix = emoji; isEmojiPickerOpen = false"
                                  :class="['w-9 h-9 flex items-center justify-center rounded-lg text-xl transition-all active:scale-90', userPrefix === emoji ? 'bg-purple-600/40 ring-1 ring-purple-500' : 'hover:bg-slate-100 dark:hover:bg-slate-700']">
                            {{ emoji }}
                          </button>
                        </div>
                      </div>
            ...
            <div class="mt-6 flex gap-3">
              <button @click="isProfileOpen = false; isEmojiPickerOpen = false" class="flex-1 bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 text-slate-900 dark:text-white font-bold py-3 rounded-xl text-sm transition-all active:scale-95">Отмена</button>
              <button @click="saveProfile" :disabled="isSaving" class="flex-1 bg-gradient-to-r from-purple-600 to-indigo-600 text-white font-bold py-3 rounded-xl text-sm shadow-lg active:scale-95 transition-all disabled:opacity-50">{{ isSaving ? 'Ждем...' : 'Сохранить' }}</button>
            </div>
          </div>
        </div>
      </div>
    ...
    <Transition name="fade">
      <div v-if="contextMenu" class="fixed inset-0 z-50" @click="closeContextMenu" @contextmenu.prevent="closeContextMenu">
        <div :style="{ position: 'fixed', left: contextMenu.x + 'px', top: contextMenu.y + 'px' }"
          ...
          <button @click="onReactFromMenu" class="w-full flex items-center gap-3 px-4 py-3 text-sm text-slate-700 dark:text-slate-200 hover:bg-slate-100 dark:hover:bg-slate-800 transition-all active:bg-slate-200 dark:active:bg-slate-700 border-t border-slate-100 dark:border-slate-800">
    ...
    <Transition name="fade">
      <div v-if="reactionPicker" class="fixed inset-0 z-50" @click="closeContextMenu">
        <div :style="{ position: 'fixed', left: reactionPicker.x + 'px', top: reactionPicker.y + 'px' }"
             class="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-2xl shadow-2xl p-2 flex gap-1" @click.stop>
          <button v-for="emoji in quickReactions" :key="emoji" @click="sendReaction(emoji)"
...
.highlight-message { animation: highlight 1.5s ease; }
@keyframes highlight {
